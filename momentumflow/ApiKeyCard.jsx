...
export default function ApiKeyCard({
  ...
  const statusText = connected
    ? 'Alpaca verified: CONNECTED'
    : configured
      ? `Keys configured, Alpaca verification failed${connectionError ? ` · ${connectionError}` : ''}`
  ...
  return (
    <div style={card}>
      ...
      <div style={{
        marginTop: 7,
        fontSize: 11,
        color: connected ? 'var(--signal-up)' : configured ? '#fbbf24' : 'var(--text-dim)',
      }}>
        {statusText}
      </div>
      ...
      {editing ? (
        <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
          <input placeholder="Alpaca Key ID" value={keyId}
            onChange={(e) => setKeyId(e.target.value)} style={input} autoComplete="off" />
          <input placeholder="Alpaca Secret Key" type="password" value={secretKey}
            onChange={(e) => setSecretKey(e.target.value)} style={input} autoComplete="off" />
          {error && <div style={{ color: 'var(--signal-down)', fontSize: 12 }}>{error}</div>}
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handleSave} disabled={busy || !keyId.trim() || !secretKey.trim()} style={btnPrimary}>
