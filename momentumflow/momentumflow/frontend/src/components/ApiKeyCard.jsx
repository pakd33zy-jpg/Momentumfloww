...
export default function ApiKeyCard({ mode, configured, keyIdMasked, connected, connectionError, onSaved }) {
  ...
  return (
    <div style={card}>
      ...
      <div style={{ marginTop: 7, fontSize: 11, color: connected ? 'var(--signal-up)' : configured ? 'var(--signal-armed)' : 'var(--text-dim)' }}>
        {connected ? 'Alpaca verified: connected' : configured ? `Keys configured, broker verification failed${connectionError ? ` · ${connectionError}` : ''}` : 'No credentials configured'}
      </div>
