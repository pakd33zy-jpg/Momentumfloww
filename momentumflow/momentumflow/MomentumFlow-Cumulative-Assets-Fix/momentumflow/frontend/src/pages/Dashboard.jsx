...
export default function Dashboard() {
  ...
  const cumulativePaperPnl = sessions
    .filter((session) => session.mode === 'paper')
    .reduce((sum, session) => sum + Number(session.total_pnl ?? session.pnl ?? 0), 0);
  const totalAssets = seedCapital + cumulativePaperPnl;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '30px' }}>
        <div style={{ background: '#1e2139', padding: '20px', borderRadius: '8px', border: '1px solid #2a2e4a' }}>
          ...
          <div style={{ fontSize: '11px', color: pnl >= 0 ? '#4ade80' : '#f87171', marginTop: '5px' }}>
            {cumulativePaperPnl >= 0 ? '+' : ''}{cumulativePaperPnl.toFixed(2)} since ${seedCapital.toFixed(2)} seed
          </div>
        </div>
